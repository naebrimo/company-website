        ...
        <div class="carousel-inner">
            @foreach($carousels as $i => $carousel)
                <div class="carousel-item @if($i == 0) active @endif">
                    ...
                    @if(!is_null($carousel['caption']))
                        <div class="carousel-caption">
                            <div class="d-block text-light p-4">
                                <h2 class="text-capitalize">{{ $carousel['caption']['title'] }}</h2>
                                <p class="text-capitalize">{{ $carousel['caption']['text'] }}</p>
                                <a class="btn btn-primary text-capitalize" href="{{ route($carousel['caption']['link']['href']) }}">{{ $carousel['caption']['link']['text'] }}</a>
                            </div>
                        </div>
                    @endif
                </div>
